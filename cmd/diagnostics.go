...
func diagnosticsCmdRun(*cobra.Command, []string) {
	...
	fmt.Fprintln(output, "")
	fmt.Fprintln(output, "          FTP Server:", redact(cfg.System.Ftp.Address), ":", cfg.System.Ftp.Port)
	fmt.Fprintln(output, "       FTP Read-Only:", cfg.System.Ftp.ReadOnly)
	fmt.Fprintln(output, "")
	...
	if !diagnosticsArgs.IncludeEndpoints {
		...
		s = strings.ReplaceAll(s, cfg.System.Ftp.Address, "{redacted}")
